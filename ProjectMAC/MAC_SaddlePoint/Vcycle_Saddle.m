function [uh, vh, ph, step, time] = Vcycle_Saddle(uh, vh, ph, f1h, f2h, gh, uTop, uBot, vLef, vRig, h, width, J, nu, gamma)
...
time = time + cputime - t0;
    ...
    function [eu, ev, ep] = VCYCLE(eu, ev, ep, r1, r2, r3, uT, uB, vL, vR, J)
        if J == 1
            ...
            err = 1;
            while (err > 1e-6)
                [eu, ev, ep] = DGS_Saddle(eu, ev, ep, r1, r2, r3, zeros(1,d+1), zeros(1,d+1), zeros(d+1,1), zeros(d+1,1), width/d, nu, gamma);
        ...
        % Presmooth
        for ii = 1:mu
            [eu, ev, ep] = DGS_Saddle(eu, ev, ep, r1, r2, r3, uT, uB, vL, vR, width/size(r1,1),nu,gamma);
        end
