function [r1, r2, r3, r4, r5, r6, r7, r8] = assembleStiffnessMatrixForPrimalDarcyInterfaceTermP1P1RT1P1DG_2(NE1, ...
...
h = 2 / n; co1 = 4/((2*psi-1)*eitaGamma); co2 = 1/eitaGamma; dof = 2;
for k = 1:n
    ...
    for j = 1:length(gcll)
        ...
        temp5 = temp5 + gcll(j) * basis_1(gpll(j), middle, bp1, ep1)' * ...
                                  basisDG1D(gpll(j), middle, 0);
        temp6 = temp6 + gcll(j) * basis_2(gpll(j), middle, bp2, ep2)' * ...
                                  basisDG1D(gpll(j), middle, 0);
        temp7 = temp7 + gcll(j) * basisDG1D(gpll(j), middle, 0)' * ...
                                  basis_1(gpll(j), middle, bp1, ep1);
        temp8 = temp8 + gcll(j) * basisDG1D(gpll(j), middle, 0)' * ...
    ...
    for alpha = 1:2
        for beta = 1:2
            r5(used3(beta),used1(alpha)) = r5(used3(beta),used1(alpha)) + temp5(alpha,beta);
            r6(used3(beta),used2(alpha)) = r6(used3(beta),used2(alpha)) + temp6(alpha,beta);
        end
    end
    for alpha = 1:dof
        for beta = 1:2
            r7(used1(beta),used3(beta)) = r7(used1(beta),used3(beta)) + temp7(alpha,beta);
            r8(used2(beta),used3(beta)) = r8(used2(beta),used3(beta)) + temp8(alpha,beta);
        end
    end   
end
