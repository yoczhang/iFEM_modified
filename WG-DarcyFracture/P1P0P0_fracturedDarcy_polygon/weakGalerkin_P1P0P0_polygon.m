...
isDomainOne = true(length(interfaceEdges), 1); isDomainOne(interfaceEdges>NE/2) = false;
interfacesEdges_domainOne = interfaceEdges(isDomainOne); interfacesEdges_domainTwo = interfaceEdges(~isDomainOne);
[i1, j1] = sort(edgeCenter(interfacesEdges_domainOne,1)); interfacesEdges_domainOne = interfacesEdges_domainOne(j1);
[i2, j2] = sort(edgeCenter(interfacesEdges_domainTwo,1)); interfacesEdges_domainTwo = interfacesEdges_domainTwo(j2);
...
[I1,I2,M,D,rhs2] = assembleInterfaceMatrix_FVMmatrix(node,edge,lGamma,eipsilon,psi,eitaGamma,NE,inteNE,edgeLength, ...
