function [soln,eqn,info] = PoissonPeriodic(node,elem,bdFlag,pde,option)
...
elem2dof = squaremesh_toP1PeriodicDof(node,elem,'x');
...
A = sparse(Ndof,Ndof);
A_priodic = zeros(Ndof,Ndof);
for i = 1:3
    for j = 1:3
        % $A_{ij}|_{\tau} = \int_{\tau}K\nabla \phi_i\cdot \nabla \phi_j dxdy$ 
        Aij = (Dphi(:,1,i).*Dphi(:,1,j) + Dphi(:,2,i).*Dphi(:,2,j)).*area;
        A = A + sparse(elem(:,i),elem(:,j),Aij,Ndof,Ndof);  
        A_priodic = A_priodic + sparse(elem2dof(:,i),elem2dof(:,j),Aij,Ndof,Ndof);  
    end
end

A_A = sparse(Ndof,Ndof);
for i = 1:3
    for j = i:3
        % $A_{ij}|_{\tau} = \int_{\tau}K\nabla \phi_i\cdot \nabla \phi_j dxdy$ 
        Aij = (Dphi(:,1,i).*Dphi(:,1,j) + Dphi(:,2,i).*Dphi(:,2,j)).*area;
        ...
        if (j==i)
            A_A = A_A + sparse(elem(:,i),elem(:,j),Aij,Ndof,Ndof);
        else
            A_A = A_A + sparse([elem(:,i);elem(:,j)],[elem(:,j);elem(:,i)],...
                           [Aij; Aij],Ndof,Ndof);        
        end        
    end
end
...
if ~isempty(pde.f) && ~isreal(pde.f)  % f is a function 
    ...
    b_priodic = accumarray(elem2dof(:),bt(:),[Ndof 1]);
end
clear pxy bt
...
A_0(p_dof0, :) = A_0(p_dof0, :) + A_0(p_dof1, :);
A_0(:, p_dof0) = A_0(:, p_dof0) + A_0(:, p_dof1);
A_0(p_dof1, :) = 0;
A_0(:, p_dof1) = 0;
b_0(p_dof0) = b_0(p_dof0) + b_0(p_dof1);
b_0(p_dof1) = 0;
...
%% |--- yc test1
A_1(p_dof0, :) = A_1(p_dof0, :) + A_1(p_dof1, :); 
b_1(p_dof0) = b_1(p_dof0) + b_1(p_dof1);
A_1(p_dof1, :) = 0;
for kk = 1:length(p_dof0)
    A_1(p_dof1(kk), [p_dof0(kk), p_dof1(kk)]) = [1, -1];
    b_1(p_dof1(kk)) = 0;
end

freeNodeplus = setdiff(freeNode, p_dof1);

cc = A_0(freeNodeplus, freeNodeplus);

x_0 = A_0(freeNodeplus, freeNodeplus) \ b_0(freeNodeplus);
x_1 = A_1(freeNode, freeNode) \ b_1(freeNode);
xx = x_0 - x_1(1:length(x_0));
